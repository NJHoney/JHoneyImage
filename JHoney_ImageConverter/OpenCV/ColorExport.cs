...
namespace JHoney_ImageConverter.OpenCV
{
    class ColorExport
    {
        ...
        public Mat SingleChannelExport(Mat matImage, int SetChannel)
        {
            ...
            Mat[] SrcArray = matImage.Split();

            Mat DstImage = new Mat(new Size(matImage.Width, matImage.Height), MatType.CV_8UC3);
            Mat[] DstArray = new Mat[3];
            Mat SrcImage = new Mat(matImage.Height, matImage.Width, MatType.CV_8UC1, new Scalar(0));
